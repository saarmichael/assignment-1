...
public class Fermat {
    ...
    private static final int ARGS_LEN = 2; //valid number of arguments

    /**
     * gets two integers n and range and prints all the Pythagorean equations s.t (1 < a,b,c < n), and
     *         a^n + b^n = c^n.
     *
    ...
    public static void main(String[] args) {
        ...
        boolean isSolutionExists = false; // variable to check if a matching triplet was found
        for (int i = 1; i <= range - 1; i++) {
            for (int j = i; j <= range - 1; j++) {
                for (int k = j; k <= range - 1; k++) {
                    /* check if i^n + j^n = k^n */
